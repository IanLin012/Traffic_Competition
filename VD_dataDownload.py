...
save_dir = os.path.join("data", "VD", f"{year:04d}", f"{month:02d}", f"{day:02d}")
...
if existing_gz_files:
    ...
    for hour in range(24):
        for minute in range(0, 60, 1):  # Assuming data every 1 minutes
            timestamp = f"{hour:02d}{minute:02d}"
            filename = f"VDLive_{timestamp}.xml.gz"
